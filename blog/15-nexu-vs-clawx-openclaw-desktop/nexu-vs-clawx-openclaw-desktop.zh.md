...
| **平台** | macOS（Apple Silicon）；Windows 和 Intel 开发中 | macOS、Windows、Linux |
...
nexu 目前支持 macOS（Apple Silicon）。Windows 和 Intel Mac 正在开发中。

ClawX 支持 macOS、Windows 和 Linux，三个平台都有预构建安装包。
...
| 平台 | macOS（Apple Silicon） | macOS、Windows、Linux |
...
如果你的工作流跑在**微信或飞书**上——你希望 AI Agent 在手机聊天里随时可用，登录即可免费使用顶级模型——nexu 就是为此设计的。

如果你今天就需要 **Windows 或 Linux** 支持，或者你的工作流需要**基于时间的定时任务**从桌面端运行，ClawX 覆盖了这些需求。

两款都是 MIT 开源、基于 OpenClaw，都在快速迭代。选择取决于你的工作流是以 IM 为中心还是以桌面为中心。
